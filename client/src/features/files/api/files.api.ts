...
import type { FileItem, FileReorderItem } from "../../../entities/file";
...
export const filesApi = {
  list: (folderId?: string) =>
    api
      .get<
        FileItem[]
      >("/files", { params: folderId ? { folderId } : undefined })
      .then((r) => r.data),
  ...
  search: (name: string) =>
    api
      .get<FileItem[]>("/files/search", { params: { name } })
      .then((r) => r.data),
  ...
  clone: (id: string) =>
    api.post<FileItem>(`/files/${id}/clone`).then((r) => r.data),
...
export const publicFilesApi = {
  getMetadata: (id: string) =>
    api.get(`/files/public/${id}`).then((r) => r.data),
  getDownloadUrl: (id: string) =>
    `http://localhost:3001/api/files/public/${id}/download`,
};
